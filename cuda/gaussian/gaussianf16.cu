...
#define DataType __nv_bfloat16
#define DataTypeString std::string("bfloat16")
...
create_matrix(DataType *m, int size){
  ...
  for (i=0; i < size; i++) {
      for (j=0; j < size; j++) {
		m[i*size+j]=__float2bfloat16(coe[size-1-i+j]);
		printf("%f", m[i*size+j]);
      }
  }
...
void InitMat(DataType *ary, int nrow, int ncol)
{
	...
	for (i=0; i<nrow; i++) {
		for (j=0; j<ncol; j++) {
			...
			*(ary+Size*i+j) = __float2bfloat16(temp[ncol*i+j]);
		}
	}  
}
...
void PrintMat(DataType *ary, int nrow, int ncol)
{
	...
	for (i=0; i<nrow; i++) {
		for (j=0; j<ncol; j++) {
			printf("%8.2f ", __bfloat162float(*(ary+Size*i+j)));
		}
		printf("\n");
	}
	printf("\n");
...
void PrintAry(DataType *ary, int ary_size)
{
	int i;
	for (i=0; i<ary_size; i++) {
		printf("%.2f ", __bfloat162float(ary[i]));
	}
	printf("\n\n");
}
void checkCUDAError(const char *msg)
...
void printVectorToFile(char *filename, DataType *ary, int ary_size)
{
	...
	for (i=0; i<ary_size; i++) {
		fprintf(fp, "%.8f ", __bfloat162float(ary[i]));
	}
	fprintf(fp, "\n\n");
	fclose(fp);
}
